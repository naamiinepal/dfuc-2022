...
def main():
    ...
    if eval_type == 0:
        ...
        eval_images = sorted(glob(os.path.join(os.path.join(eval_data_base_dir_1, images_path), "*.jpg")))
        eval_loader = data_loader_d(val_imtrans_d(), eval_batch_size, os.path.join(eval_data_base_dir_1, images_path), os.path.join(eval_data_base_dir_1, masks_path))
        ...
        if eval_type == 1:
            ...
            with open('outputs/csv_files/dice_scores_all' + eval_name  + '.csv', 'w') as csv_file:
                ...
                for items in list_for_csv:
                    writer_csv.writerow([items[0], items[1].item()])
